...
# --- Proměnné pro PENÍZE (Equity) ---
kapital = 10000  # Začínáme s 10 000 USD
equity_historie = [kapital] # Sem budeme ukládat stav účtu po každém obchodu
equity_datumy = [weekly_df.index[0]] # Datumy pro graf

# Seznamy pro grafy vstupů
signal_entry_long = []
signal_entry_short = []
signal_tp = []
signal_sl = []
...
for i in range(2, len(weekly_df)):
    ...
    if pozice == 0:
        ...
        # 2. Rušení setupu (Tvoje bezpečná varianta - nejdřív kontrolujeme Close)
    ...
    # ---------------------------------------------------
    # B) POKUD JSME V POZICI (Výpočet zisku/ztráty)
    # ---------------------------------------------------
    elif pozice == 1: # LONG
        if low <= moje_sl_cena: # Stop Loss
            ...
            print(f"❌ LONG SL | Zůstatek: {kapital:.0f} USD")
        
        elif high >= moje_tp_cena: # Take Profit
            pozice = 0
            signal_tp.append(datum)
            kapital = kapital * 1.10 # Vydělali jsme 10%
            equity_historie.append(kapital)
            equity_datumy.append(datum)
            print(f"💰 LONG TP | Zůstatek: {kapital:.0f} USD")

    elif pozice == -1: # SHORT
        if high >= moje_sl_cena: # Stop Loss
            pozice = 0
            signal_sl.append(datum)
            kapital = kapital * 0.98 # Ztráta 3%
            equity_historie.append(kapital)
            equity_datumy.append(datum)
            print(f"❌ SHORT SL | Zůstatek: {kapital:.0f} USD")
            ...
            equity_historie.append(kapital)
            equity_datumy.append(datum)
            print(f"💰 SHORT TP | Zůstatek: {kapital:.0f} USD")

# Výpis výsledku
zisk_procenta = ((kapital - 10000) / 10000) * 100
print("-" * 40)
print(f"KONEC TESTU. Konečný kapitál: {kapital:.0f} USD")
print(f"Celkové zhodnocení: {zisk_procenta:.2f} %")

# --- GRAFY (Dva pod sebou) ---
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10), sharex=True)

# Graf 1: Cena a Signály
...
ax1.set_title("Strategie a Signály")
ax1.legend()
ax1.grid(True, alpha=0.3)

# Graf 2: Vývoj peněz (Equity Curve)
ax2.plot(equity_datumy, equity_historie, color="green", linewidth=2, marker="o")
ax2.axhline(y=10000, color='red', linestyle='--', label="Startovní čára") # Červená čára startu
ax2.set_title("Vývoj kapitálu (Equity Curve)")
ax2.set_ylabel("USD")
